...
client:on('messageCreate', function(message)
	...
	cmd = cmd or message.content
	if cmd == '>stats' then
		message.channel:sendMessage(string.format(' ``` Name: JBot#7214. Bot: Yes. Desc: Multipurpose bot used to moderate servers or provide fun commands for you and your server. Language: Lua. Framework: Luvit. Lib: Discordia. Lib Creator: Sinister Rectus. Region: World. Version: 1.4.3. Updated: Yes. Updates: No. ```, %s', message.author:getMentionString()))
		end
	end
end)
...
client:on('messageCreate', function(message)
	...
	cmd = cmd or message.content
	if cmd == '>cmds' then
		message.channel:sendMessage(string.format(' ``` Comamnds: @Creator: >test, >servers, >stats, >disconnect, >todo, >setgame, >setidle, >setonline @Admin: >kick <@user>, >ban <@user>, >warn <@user> >admins, >creator, >clear <All> @Free: >cmds, >joke, >invite, >help, >about, >home, >new ```, %s', message.author:getMentionString()))
...
client:on('messageCreate', function(message)
	...
	cmd = cmd or message.content
	if cmd == '>new' then
		message.channel:sendMessage(string.format(' ``` Current Version 1.4.3: Fixed >clear <all>. Shouldnt cause anymore issues, sorry about that.```, %s', message.author:getMentionString()))
